...
int gameSDL::titleScreen(SDL_Renderer* des)
{
    ...
    while (SDL_PollEvent(&g_event) > 0)
    {
        if (g_event.type == SDL_QUIT) return 3;
            ...
            case SDLK_RETURN:
                ...
                switch (selectedMenu)
                {
                case 0: Mix_HaltMusic();return 1; break;
    ...
    TitleGame.RenderText(des,(SCREEN_WIDTH-TitleGame.GetRect().w)/2,50);

    int height = 70 + TitleGame.GetRect().h;
    for (int i=0; i<MenuOption; ++i)
    {
        TitleChoice[i].RenderText(des,(SCREEN_WIDTH-TitleChoice[i].GetRect().w)/2,height);
...
int gameSDL::End_Message(SDL_Renderer* des, const string& str, TTF_Font* font)
{
    Results.SetText(str);
    ...
    black_bg.Render(des);
    Results.RenderText(des, (SCREEN_WIDTH-Results.GetRect().w)/2,50);

...
ThreatsObject:: ~ThreatsObject()= default;
...
bool ThreatsObject::LoadImgs(std::string path, SDL_Renderer *screen) {
    bool ret = BaseObject::LoadImg(path, screen);
    if(ret){
...
void ThreatsObject::Show(SDL_Renderer *des) {
    if(come_back_time_ == 0){
        ...
        SDL_RenderCopy(des, p_objcet_, currentClip, &rendQuad);
    }
}
void ThreatsObject::Shows(SDL_Renderer *des) {
    if(come_back_time_ == 0){
        rect_.x = x_pos_ - map_x_;
        rect_.y = y_pos_ - map_y_;
        frame_++;
...
void ThreatsObject::CheckToMap(Map &g_map) {
    ...
    if(x1 >= 0 && x2 < MAX_MAP_X && y1 >= 0 && y2 < MAX_MAP_Y){
        ...
        if(y_val_ > 0){
            ...
            int val2 = g_map.tile[y2][x2];
            if((val1 != BLANK_TILE && val2 != COIN) || (val2 != BLANK_TILE && val2 != COIN)) {
                y_pos_ = y2 * TILE_SIZE;
...
void ThreatsObject::InitBullet(BulletObject *p_bullet, SDL_Renderer *screen) {
    if(p_bullet != nullptr){
        p_bullet->set_bullet_type(BulletObject::X_BULLET);
